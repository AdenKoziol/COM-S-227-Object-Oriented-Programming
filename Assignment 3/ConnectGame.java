...
public class ConnectGame {
	...
	public void tryContinueSelect(int x, int y) 
	{
		...
		/*if(goBack != -1)
		{
			for(int i = tilesSelected.size() - 1; i > 0; i++)
			{
	...
	public void upgradeLastSelectedTile() 
	{
		...
		for(int i = 0; i < gameGrid.getWidth(); i++)
		{
			for(int j = 0; j < gameGrid.getHeight(); j++)
			{
				if(gameGrid.getTile(i, j).getLevel() > getMaxTileLevel())
				{
					maxTile++;
					minTile++;
					dialogListener.showDialog("New block " + Math.pow(2, maxTile) + ", removing blocks " + Math.pow(2, minTile - 1));
					for(int k = 0; k < gameGrid.getWidth(); k++)
					{
						for(int l = 0; l < gameGrid.getHeight(); l++)
						{
							if(gameGrid.getTile(k, l).getLevel() < getMinTileLevel())
							{
								tilesSelected.add(gameGrid.getTile(k, l));
							}
						}
					}
					dropSelected();
	...
	public void dropLevel(int level) 
	{
		for(int i = 0; i < gameGrid.getWidth(); i++)
		{
			for(int j = 0; j < gameGrid.getHeight(); j++)
			{
				...
				{
					tilesSelected.add(gameGrid.getTile(i, j));
				}
			}
		}
		tilesSelected.add(tilesSelected.get(tilesSelected.size() - 1));
		dropSelected();
	}
